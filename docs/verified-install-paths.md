...
Expected local result for v1.0.0:

- Claude Code: `gooblin@gooblin`, version `1.0.0`, status `enabled`.
- Codex CLI: `gooblin@gooblin`, version `1.0.0`, status `installed, enabled`.
...
```bash
codex plugin marketplace add jsleemaster/gooblin --ref release/v1.0.0
codex plugin add gooblin@gooblin
codex plugin list
```

Result: `gooblin@gooblin`, version `1.0.0`, status `installed, enabled`.
