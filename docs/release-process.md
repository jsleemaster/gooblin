...
6. Only after the attachment check, approve the waiting `npm-release` environment deployment. The `publish` job downloads the same Actions artifact, verifies its checksum, metadata, package version, and source SHA, then runs `npm publish <tgz>` with OIDC as its final mutating command. It must not run a second `npm pack`.
